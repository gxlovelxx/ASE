...
"""
    数据接收函数
"""
def receiver(config32,config512,mcast_group_ip,mcast_group_port,dst_ip,dst_port1,dst_port2):
                ...
                for Param32Config in config32:
                    if hex(SegmentData[1]) == hex(int(Param32Config.InitialFrame)-1):  #SFID验证
                       #获取数据
                        ...
                        #print(paradata)
                        data_bin = hex2bin(paradata, int(Param32Config.EndWord) - int(Param32Config.StartWord) + 1).zfill(16)
...
if __name__ == "__main__":
    ...
    receiver(config32,config512,config_net[0],int(config_net[1]),config_net[2],int(config_net[3]),int(config_net[4]))
